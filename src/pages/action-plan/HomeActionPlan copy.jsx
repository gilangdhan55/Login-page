        ...
        >
           ...
           <ul className='menu-inner overflow-y-auto overflow-x-hidden flex-grow-1 h-screen py-2 px-4 pb-[4.3rem] relative'>
                <li className='menu-item my-2 mx-0 will-change-transform'>
                    <Link to='/action-plan' className={`menu-link group text-base my-0 mx-[-7px] ${location.pathname.startsWith("/action-plan") ? 'active' : ''} flex items-end px-4 py-2 rounded-md relative`} >
                        <IconHome strokeWidth={1.5} className='menu-icon mr-2 min-w-[22px] w-[22px] object-contain group-[.active]:text-white'/> 
                        <span className='menu-text text-ellipsis whitespace-nowrap leading-[1.467] overflow-hidden  group-[.active]:text-white'>Home</span>
                    </Link>
                </li>
                <li className='menu-header transition-opacity duration-300 ease-in-out my-0 mx-[-5px] flex items-end text-base uppercase px-4 py-2'>
                    <span className='menu-header-text uppercase tracking-[0.4px] whitespace-nowrap text-gray-500 '>Action Plan</span>
                </li>
                {lengthMap.map((item, index) => (
                    <li key={index} className='menu-item my-2 mx-0 will-change-transform'>
                        <Link to={`/action-plan-${index}`} className={`menu-link text-base my-0 mx-[-7px] flex items-end px-4 py-2 rounded-md relative ${location.pathname.startsWith("/action-plan-"+index) ? 'active' : ''}`} >
                            <IconHome strokeWidth={1}  className='menu-icon mr-2 min-w-[22px] w-[22px] object-contain'/> 
                            <span className='menu-text text-ellipsis whitespace-nowrap leading-[1.467] overflow-hidden '>Home</span>
                        </Link>
                    </li>
                ))} 
           </ul>
